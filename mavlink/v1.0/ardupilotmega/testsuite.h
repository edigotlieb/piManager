...
static void mavlink_test_pi_trigger(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	...
	mavlink_pi_trigger_t packet1, packet2;
        ...
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_pi_trigger_encode(system_id, component_id, &msg, &packet1);
